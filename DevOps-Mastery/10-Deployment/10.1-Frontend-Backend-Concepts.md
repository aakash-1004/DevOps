...
REST API = a way for systems to talk to each other over HTTP, using standard verbs to act on "resources" (data).

**Core idea**: Everything is a resource, addressed by a URL. You act on it using HTTP methods:
...
**Key traits that make it "REST"**:

- **Stateless** — each request carries everything needed (auth token, params). Server doesn't remember you between calls.
- **Resource-based URLs** — nouns, not actions. `/users/123` not `/getUser?id=123`.
- **Standard status codes** — 200 (ok), 201 (created), 400 (bad request), 401 (unauthorized), 404 (not found), 500 (server error). You'll live and die by these when debugging.
- **JSON is the default payload format** these days (used to be XML more often).
### HTTP Methods
...
- [[10.4-Deployment-Debugging]]
- [[10.6-Apache-vs-Nginx]]